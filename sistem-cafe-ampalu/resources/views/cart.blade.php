            ...
            @if(session('cart') && count(session('cart')) > 0)
                @foreach(session('cart') as $id => $details)
                    ...
                    <div x-data="{ 
                            ...
                            itemId: '{{ $id }}', // <-- KITA SIMPAN ID ITEM DI SINI
                            updateCart(newQuantity) {
                                ...
                                fetch('{{ route('cart.update') }}', {
                                    ...
                                    body: JSON.stringify({
                                        id: this.itemId, // Menggunakan ID yang sudah disimpan
                                ...
                                .then(data => {
                                    if(data.success) {
                                        ...
                                        // ===================================================
                                        // ========= PENDEKATAN BARU UNTUK MENGHAPUS =========
                                        // ===================================================
                                        if (newQuantity === 0) {
                                            // 1. Cari elemen berdasarkan ID uniknya
                                            let itemElement = document.getElementById('cart-item-' + this.itemId);
                                            
                                            // 2. Jika elemen ditemukan, hapus
                                            if (itemElement) {
                                                itemElement.remove();
                                            }
                                        }
                        ...
                        id="cart-item-{{ $id }}" {{-- ID unik untuk setiap item --}}
        ...
        <div class="w-full max-w-2xl mx-auto text-gray-800 font-semibold 
            ...
            <div class="flex flex-col space-y-2 text-left w-full max-w-xs">
                <div class="flex items-center space-x-3">
                    ...
                    <span>NOMOR MEJA ANDA : {{ session('table_number', 'N/A') }}</span>
                </div>
                <div class="flex items-center space-x-3">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8V6m0 12v-2m0-10a9 9 0 110 18 9 9 0 010-18z" />
                    </svg>
                    <span>TOTAL 
